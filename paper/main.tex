...
该模型对每个输入维度独立维护状态空间, 并通过sigmoid门控实现维度间的轻量级交互, 以增强对高维动力学系统的表达能力.
在Gymnasium MuJoCo基准的Humanoid和HumanoidStandup两个人形机器人数据集上的多种子实验表明, MIMO-WM在Humanoid上取得最优精度, 均方误差为$19.87\times10^{-2}$, 比次优方法降低1.5\%; 参数量仅0.138M, 比次优方法减少38.4\%.
理论分析证明了对角状态空间模型递推与卷积模式的等价性、计算复杂度优势以及交叉熵方法采样式模型预测控制的收敛性.
实验结果验证了MIMO-WM在人形机器人状态预测任务中的有效性和轻量化优势.代码已开源在https://github.com/SEMHAQ/MIMO-WM.
...
Humanoid robot state prediction faces the dual challenges of accuracy and computational efficiency. This paper proposes MIMO-WM, a lightweight world model based on multi-input multi-output state space model architecture, which maintains an independent state space for each input dimension and introduces lightweight cross-dimension interaction via a sigmoid gating mechanism for enhanced expressiveness on high-dimensional dynamics. Multi-seed experiments on Gymnasium MuJoCo Humanoid and HumanoidStandup benchmarks demonstrate that MIMO-WM achieves the best prediction accuracy on Humanoid with mean squared error of $19.87\times10^{-2}$, outperforming the second-best method by 1.5 percent, while requiring only 0.138 million parameters, 38.4\% fewer than the second-best method. Theoretical analysis proves the equivalence of recurrent and convolution computation modes, the computational complexity advantage, and the convergence guarantee of cross-entropy method based model predictive control. Experimental results validate the effectiveness and lightweight advantage of MIMO-WM for humanoid robot state prediction tasks.The code is open source at https://github.com/SEMHAQ/MIMO-WM. 
...
Nagabandi等人\textsuperscript{\cite{15,16,17}}展示了神经网络动力学模型在MPC中的有效性.
Chua等人\textsuperscript{\cite{24}}提出了PETS方法, 通过概率集成模型处理模型不确定性.
Hafner等人\textsuperscript{\cite{13}}的Dreamer方法则将潜在动力学模型与MPC相结合.
...
综合两个数据集上的实验结果, MIMO-WM在Humanoid上取得最优精度, MSE为$19.87\times10^{-2}$, 比Mamba-WM降低1.5\%, 比Transformer-WM降低29.3\%; 在HumanoidStandup上, MIMO-WM的MSE为$53.10\times10^{-2}$, $R^2$为0.444, 与Transformer并列最优, 但参数量仅0.138M, 为Transformer的45.7\%, 在所有方法中最为轻量.
...
本文提出了多输入多输出世界模型MIMO-WM, 通过MIMO-SSM与sigmoid门控机制的结合, 在保持线性计算复杂度的同时实现了更强的表达能力.
