...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\makeatother
\title{\LARGE\textbf{MIMO-WM: 面向人形机器人的轻量级状态空间世界模型}\\
...
\date{}
\fancyhead[CO]{{\footnotesize 周新民等: MIMO-WM轻量级状态空间世界模型}}
...
针对人形机器人高维状态预测中轻量化与精度的双重需求, 本文提出一种轻量级世界模型MIMO-WM.
该模型对每个输入维度独立维护状态空间, 并通过sigmoid门控引入维度间的轻量级交互.
参数量仅0.138M, 在各对比方法中最为轻量.
在Humanoid数据集上, MIMO-WM的均方误差为0.199, 优于Mamba-WM的0.202和Transformer-WM的0.281; 在HumanoidStandup数据集上, 以0.138M参数达到与Transformer的0.302M参数相当的预测精度.
消融实验表明门控机制可降低MSE约8.9\%, 序列长度分析表明输入序列长度$T{=}8$时预测精度最优.
将MIMO-WM嵌入模型预测控制框架, 交叉熵方法采样式求解器相比梯度求解器可提升控制频率1到2个数量级; MIMO-WM在Humanoid上实现了7.99Hz的控制频率, 且其3步预测均方误差为0.216, 在对比方法中最低.
复杂度分析表明, 设层数$L$、隐空间维度$D$时, MIMO-WM的复杂度为$O(LTD\log T)$, 当$T{=}32$时相比Transformer的$O(T^2D)$约可降低计算量6倍.
代码已开源在https://github.com/SEMHAQ/MIMO-WM.

{\CJKfamily{hei} 关键词:} 状态空间模型; 世界模型; 状态预测; 模型预测控制; 具身智能

{\CJKfamily{hei}引用格式:} 周新民, 余焕杰, 张慧慧, 等. MIMO-WM: 面向人形机器人的轻量级状态空间世界模型. 控制理论与应用, xxxx, xx(x): xxx -- xxx\\\vspace*{-16pt}
...
\begin{center}
\parbox{\textwidth}{\setlength{\parindent}{1em}
{\centering\large\textbf{MIMO-WM: A Lightweight State Space World Model for Humanoid Robots}} \vspace{-1.2mm}
...
Humanoid robot state prediction requires both lightweight design and prediction accuracy. This paper proposes MIMO-WM, a lightweight world model that maintains an independent state space for each input dimension and introduces cross-dimension interaction via a sigmoid gating mechanism. With only 0.138 million parameters, MIMO-WM is the most lightweight among all compared methods. Multi-seed experiments on Humanoid show that MIMO-WM achieves a mean squared error of 0.199, outperforming Mamba-WM at 0.202 and Transformer-WM at 0.281; on HumanoidStandup, it delivers accuracy comparable to Transformer while using only 45.7\% of its parameters. Ablation studies show that the gating mechanism reduces MSE by 8.9\%, and sequence length analysis shows that an input length of $T{=}8$ yields the best prediction accuracy. When integrated into a model predictive control framework, the cross-entropy method achieves 1 to 2 orders of magnitude higher control frequency than gradient-based solving; MIMO-WM achieves 7.99 Hz on Humanoid with the a 3-step prediction MSE of 0.216, lower than other compared methods. Complexity analysis with $L$ layers and hidden dimension $D$ shows that MIMO-WM achieves $O(LTD\log T)$ complexity, offering approximately 6$\times$ reduction compared to Transformer's $O(T^2D)$ when $T{=}32$. The code is open source at https://github.com/SEMHAQ/MIMO-WM.

\textbf{Key words:} state space model; world model; state prediction; model predictive control; embodied intelligence

\textbf{Citation:} X. Zhou, H. Yu, H. Zhang, et al. MIMO-WM: A lightweight state space world model for humanoid robots. {\it Control Theory \& Applications}, xxxx, xx(x): xxx -- xxx}}
\end{center}
...
人形机器人作为具身智能的重要载体, 其高维度状态空间、
全向平衡约束及多关节耦合动力学使得``感知--决策--控制"的闭环实现极具挑战.
其中, 世界模型\textsuperscript{\cite{2}}作为环境动力学的内部表征, 其核心功能是通过状态转移建模实现对未来状态的预测, 并基于预测能力支持规划与决策.
...
近期, Mamba策略网络\textsuperscript{\cite{10,11}}和SSM时序预测\textsuperscript{\cite{12}}等工作开始将SSM引入机器人和序列决策领域,
但前者侧重于策略学习, 后者侧重于时序数据预测,
均未涉及面向机器人状态预测的世界模型构建.
人形机器人状态预测任务面临三重挑战: 状态维度高达348$\sim$376维, 涉及全身多关节的强耦合动力学; 双足支撑切换导致动作-状态映射具有时变非线性和非连续性; 且实时控制回路要求毫秒级的推理延迟.
这些特点要求世界模型在保持高预测精度的同时, 具备极低的计算开销.

更重要的是, 现有SSM方法在应用于人形机器人状态预测时存在根本性局限:
S4D采用固定对角状态矩阵, 无法根据输入内容调整动力学特性,
而人形机器人的动力学参数如足地接触刚度、惯性矩会随支撑相切换和关节配置变化;
...
1)~~提出多输入多输出世界模型（Multi-Input Multi-Output World Model, MIMO-WM）, 通过MIMO-SSM与sigmoid门控机制的结合, 在保持线性计算复杂度的同时实现更强的表达能力. 门控机制使模型能够根据输入内容动态调节信息流动, 参数量仅0.138M.

2)~~在Gymnasium MuJoCo基准的Humanoid和HumanoidStandup两个人形机器人数据集上进行了多种子实验. 结果表明MIMO-WM在Humanoid上取得优于对比方法的精度, 均方误差为$19.87\times10^{-2}$, 比Mamba-WM降低1.5\%, 比Transformer-WM降低29.3\%, 同时参数量最少;

3)~~在机器人世界模型状态预测任务上系统比较了LSTM、GRU、Transformer、Mamba、TCN和MIMO-WM六种序列模型, 分析了门控机制对SSM表达能力的提升效果;
...
\subsection{人形机器人世界模型}

世界模型的概念最早可追溯到Karl Friston的主动推理理论,
其核心思想是智能体通过构建环境的内部模型来进行预测和规划.
在深度学习时代, Ha和Schmidhuber提出了基于变分自编码器VAE和循环神经网络的世界模型框架,
通过学习压缩的环境表征来实现策略学习.
Hafner等人\textsuperscript{\cite{13}}提出了Dreamer系列方法, 利用潜在动力学模型在想象中进行策略优化,
在多个连续控制基准上取得了领先性能.
DreamerV3\textsuperscript{\cite{14}}进一步统一了离散和连续动作空间的处理, 展示了世界模型的通用性.

在人形机器人领域, 动力学模型与模型预测控制相结合的思路已被广泛探索\textsuperscript{\cite{15,16,17}}.
然而, 人形机器人的高维状态空间和双足动态平衡特性对世界模型的预测精度和实时性提出了苛刻要求.
近期, RT-2将大规模视觉-语言模型与机器人动作空间对齐,
展示了基础模型在机器人控制中的潜力.
然而, 这些方法普遍面临计算开销大、难以满足实时控制要求的问题.
在高效计算方面, TinyML\textsuperscript{\cite{18}}和模型压缩技术为资源受限平台上的深度学习部署提供了解决方案.
在仿真到真实迁移方面, 域随机化和快速电机适应RMA等方法缩小了仿真与真实机器人之间的差距.
世界模型在自动驾驶领域也得到了广泛关注\textsuperscript{\cite{19}}, 连续-离散状态空间模型为混合系统建模提供了新思路.
本文提出的MIMO-WM旨在以极低的计算开销实现高质量的状态预测,
为机器人状态预测提供一种轻量级的MIMO-SSM解决方案.

\subsection{状态空间模型}

SSM源于控制理论中的线性系统描述,
近年来在深度学习领域引起了广泛关注.
Gu等人\textsuperscript{\cite{20}}提出的HiPPO框架通过最优多项式投影解决了连续时间记忆问题,
...
在保持性能的同时显著降低了实现复杂度.
并行扫描算法\textsuperscript{\cite{21}}也被用于加速SSM的训练过程.
H3\textsuperscript{\cite{22}}结合了SSM与门控注意力, 在语言建模中取得了与Transformer相当的性能.

Mamba是SSM发展的一个重要里程碑.
它引入了选择性扫描机制, 使SSM的状态更新能够根据输入内容动态调整,
从而在保持线性计算复杂度的同时实现了与Transformer相当的建模能力.
Mamba的块结构已成为现代SSM架构的标准设计模式.
Mamba-2进一步揭示了SSM与注意力机制之间的结构化状态空间对偶性,
...
其中Mamba策略网络在连续控制任务中取得了与Transformer策略网络相当的性能,
但推理速度提升了2--3倍.
在时序预测方面, SSM被用于长程多变量时序预测, 利用其线性复杂度和长程建模优势.
在世界模型方面, Dreamer系列和TD-MPC等方法主要基于循环神经网络或潜在动力学模型进行环境建模,
且主要面向视觉输入, 计算开销较大.
...
SSM类方法通过线性递推结构实现了$O(T)$的每步推理复杂度和$O(1)$的单步延迟,
在长序列场景下具有显著的计算优势.
...
Nagabandi等人展示了神经网络动力学模型在MPC中的有效性.
Chua等人\textsuperscript{\cite{24}}提出了PETS方法, 通过概率集成模型处理模型不确定性.
Hafner等人的Dreamer方法则将潜在动力学模型与MPC相结合.
...
\end{eqnarray}
使得给定历史观测序列$\{{\bm s}_{0:T-1}, {\bm a}_{0:T-1}\}$,
能够预测未来$H$步的状态轨迹$\hat{{\bm s}}_{T:T+H-1}$.
...
MIMO-WM的核心是多输入多输出状态空间模型（multi-input multi-output state space model, MIMO-SSM）架构. 传统SSM为单输入单输出（single-input single-output, SISO）结构,
每个隐藏维度独立维护一个标量状态.
...
\begin{eqnarray}
\hat{{\bm s}}_T = {\bm W}_d{\bm z}_{T-1}' + {\bm b}_d + {\bm s}_{T-1},
\end{eqnarray}
...
SSM输出由${\bm y}_t^{(d)} = {\bm C}^{(d)}{\bm h}_t^{(d)} + {\bm D}^{(d)}\tilde{z}_t^{(d)}$计算, 记${\bm y}_t$为SSM的输出向量. ${\bm y}_t$经sigmoid门控调节:
\begin{equation}
{\bm o}_t = {\bm y}_t \odot \sigma({\bm W}_g\tilde{{\bm z}}_t + {\bm b}_g),
\end{equation}
...
并行SSM${\bm y} \leftarrow \text{DiagSSM}(\tilde{{\bm z}})$,
门控${\bm o} \leftarrow {\bm y} \odot \sigma({\bm W}_g\tilde{{\bm z}} + {\bm b}_g)$,
...
当序列较长($T > D/\log D$)时, MIMO-WM在计算效率上更具优势.
\end{Thm}
...
\odot {\bm y}_t \odot \sigma'(\cdot) \tilde{{\bm z}}_t^{\rm T},
\end{equation}
其中$\sigma'$为sigmoid的导数. 该梯度受SSM输出${\bm y}_t$调控, 使门控能自适应学习何时信任SSM预测.
\end{Cor}
...
\mathcal{L}_{\text{m}} &= \frac{1}{H}\sum_{h=0}^{H-1}\|{\bm s}_{T+h} - \hat{{\bm s}}_{T+h}\|^2, \\
...
对比方法包括LSTM-WM、GRU-WM、Transformer-WM、Mamba-WM和TCN-WM, 所有模型统一采用隐空间维度$D{=}96$, 层数$L{=}2$确保公平比较. MIMO-WM采用$D{=}96$, $N{=}16$, 层数$L{=}2$的配置. 所有实验使用5个随机种子42, 123, 456, 789, 1024运行并报告均值和标准差. 评价指标为MSE、$R^2$、推理时间和参数量. 训练采用100个epoch的AdamW优化器, 学习率$5\times10^{-4}$, 余弦退火调度, 序列长度$T=32$, 批量大小1024, 梯度裁剪阈值1.0. 实验在NVIDIA RTX 3090 GPU上进行.
...
表~1展示了各模型在Humanoid数据集上的性能对比. MIMO-WM的精度优于各对比方法, MSE为$19.87\times10^{-2}$, 低于Mamba-WM的$20.18\times10^{-2}$(相对降低1.5\%), 低于TCN-WM的$20.68\times10^{-2}$(降低3.9\%), 低于Transformer-WM的$28.11\times10^{-2}$(降低29.3\%), 低于GRU-WM的$36.60\times10^{-2}$和LSTM-WM的$39.93\times10^{-2}$.
值得注意的是, MIMO-WM的参数量仅0.138M, 低于TCN-WM的0.189M和Mamba-WM的0.224M, 在所有方法中最为轻量.
这表明MIMO-WM在保持轻量化的同时在预测精度上具有优势.
...
综合两个数据集上的实验结果, MIMO-WM在Humanoid上的精度优于各对比方法, MSE为$19.87\times10^{-2}$, 比Mamba-WM降低1.5\%, 比Transformer-WM降低29.3\%; 在HumanoidStandup上, MIMO-WM的MSE为$53.10\times10^{-2}$, $R^2$为0.444, 与Transformer精度相当, 但参数量仅0.138M, 为Transformer的45.7\%, 在所有方法中最为轻量.
...
\midrule
8  & 20.14$\pm$0.13 & 0.765 & 50.02$\pm$0.03 & 0.478 \\
...
Humanoid数据集上的实验结果表~1表明MIMO-WM的精度优于各对比方法, MSE为$19.87\times10^{-2}$, 低于Mamba-WM的$20.18\times10^{-2}$和Transformer-WM的$28.11\times10^{-2}$.
...
本文存在以下局限性:
1)~~模型仅支持单步状态预测, 在多步自回归展开中误差会累积, 限制了长期规划能力;
2)~~当前仅处理状态-动作序列, 未涉及视觉输入, 难以应用于视觉-运动联合决策场景.

\section{结论}

本文提出了基于维间独立状态空间的人形机器人轻量级世界模型MIMO-WM. 实验结果表明, 对于人形机器人这类高维物理动力学系统, 维度间独立SSM加轻量级门控交互的设计能够以极低的参数开销达到与全混合架构Transformer、Mamba相当甚至更优的预测精度. 这说明在物理动力学预测中, 全维度混合并非必要, 每个关节和自由度具有相对独立的动力学特性, 先独立建模再轻量交互的策略更符合问题的内在结构. 本工作为资源受限平台上的机器人状态预测提供了一种兼顾精度与轻量化的方案, 也为SSM在具身智能领域的应用提供了实验依据.

未来工作将聚焦以下方向:
1)~~探索多步预测中的误差校正机制, 提升长期规划可靠性;
2)~~融合视觉编码器, 构建状态-视觉多模态世界模型.
...
\bibitem{10}SHANG J, LI X, ZHANG Y, et al. Mamba policy: Selective state space models for robot visuomotor policy. {\it IEEE Robotics and Automation Letters}, 2025, 10(3): 2345--2352.
\bibitem{11}OTA T. Decision Mamba: Reinforcement learning via sequence modeling with selective state spaces. {\it arXiv preprint arXiv:2403.19925}, 2024.
\bibitem{12}AHAMED M A, CHENG Q. TimeMachine: A time series is worth 4 Mambas for long-term forecasting. {\it European Conference on Artificial Intelligence}, 2024: 1688--1695.
\bibitem{13}HAFNER D, LILLICRAP T, BA J, et al. Dream to control: Learning behaviors by latent imagination. {\it Advances in Neural Information Processing Systems}, 2019, 32: 1217--1228.
...
\bibitem{15}NAGABANDI A, KAHN G, FEARING R S, et al. Neural network dynamics for model-based deep reinforcement learning with model-free fine-tuning. {\it IEEE International Conference on Robotics and Automation}, 2018: 3543--3550.
\bibitem{16}ZHANG Yi-gang, HUANG Dao-ping, LIU Yi-qi. Stochastic model predictive control for discrete time-varying uncertain systems with chance constraint. {\it Control Theory \& Applications}, 2025, 42(12): 2459--2467.\\(张逸刚, 黄道平, 刘乙奇. 受概率约束的离散时变不确定系统的随机模型预测控制. 控制理论与应用, 2025, 42(12): 2459--2467.)
\bibitem{17}WANG Juan, ZHAO Cheng-jing, YANG Zhi-jie. Trajectory tracking control of quadcopter UAV based on MPC iterative learning. {\it Control Theory \& Applications}, 2025, 42(12): 2528--2534.\\(王娟, 赵成璟, 杨智杰. 基于MPC迭代学习的四旋翼无人机轨迹跟踪控制. 控制理论与应用, 2025, 42(12): 2528--2534.)
\bibitem{18}WARDEN P, SITUNAYAKE D. TinyML: Machine learning with TensorFlow Lite on Arduino and ultra-low-power microcontrollers. {\it O'Reilly Media}, 2019.
\bibitem{19}HU Y, WANG W, LI L, et al. A survey on world models for autonomous driving. {\it arXiv preprint arXiv:2403.02622}, 2024.
\bibitem{20}GU A, DAO T, ERMON S, et al. HiPPO: Recurrent memory with optimal polynomial projections. {\it Advances in Neural Information Processing Systems}, 2020, 33: 1474--1487.
\bibitem{21}SMITH J T, WARRINGTON A, LINDERMAN S W. Simplified state space layers for sequence modeling. {\it International Conference on Learning Representations}, 2023.
\bibitem{22}FU D, DAO T, SAAB K, et al. Hungry hungry hippos: Towards language modeling with state space models. {\it International Conference on Learning Representations}, 2023.
...
{\CJKfamily{kai} 作者简介:}\\
 {\scriptsize
\indent \textbf{周新民}~~~~博士, 教授, 博士生导师, CCF杰出会员, 主要从事具身智能、数据智能、AI大模型等方面的研究. E-mail: \mbox{zhouxinmin2699@163.com};\\
