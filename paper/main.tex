...
世界模型的概念可追溯到主动推理理论, 其核心是智能体通过构建环境内部模型进行预测和规划.
Ha和Schmidhuber提出了基于变分自编码器和循环神经网络的世界模型框架.
Hafner等人\textsuperscript{\cite{10}}提出Dreamer系列方法, 利用潜在动力学模型在想象中进行策略优化.
DreamerV3\textsuperscript{\cite{11}}进一步统一了离散和连续动作空间处理.
Nagabandi等人\textsuperscript{\cite{12,13,14}}将神经网络动力学模型与MPC相结合, 实现了机器人腿部运动快速适应.
然而, 这些方法普遍面临计算开销大的问题.
世界模型在自动驾驶领域也得到了广泛关注\textsuperscript{\cite{16}}.
本文提出的MIMO-WM旨在以极低的计算开销实现高质量状态预测.

\subsection{状态空间模型}

SSM源于控制理论的线性系统描述.
Gu等人\textsuperscript{\cite{17}}提出HiPPO框架, 为SSM在序列建模中的应用奠定理论基础.
S4通过结构化参数化解决了长程依赖建模的数值稳定性问题.
S4D简化了对角SSM参数化, 直接使用复数对角矩阵.
并行扫描算法\textsuperscript{\cite{18}}也被用于加速SSM训练.
H3\textsuperscript{\cite{19}}结合SSM与门控注意力, 取得了与Transformer相当的性能.

Mamba引入选择性扫描机制, 使SSM状态更新能够根据输入内容动态调整, 在保持线性复杂度的同时实现了与Transformer相当的建模能力.
Mamba-2进一步揭示了SSM与注意力机制之间的结构化状态空间对偶性.

现有SSM方法应用于机器人状态预测时存在局限: S4D采用固定对角参数化, 无法根据输入调整动力学特性; Mamba的计算开销较大.
MIMO-SSM通过门控机制进行自适应调整, 参数量和计算开销远小于Mamba.

近期SSM类方法开始在机器人领域得到应用, 包括Mamba策略网络和SSM时序预测.
在世界模型方面, Dreamer系列和TD-MPC等方法主要基于循环神经网络或潜在动力学模型, 面向视觉输入, 计算开销较大.
RoboMamba\textsuperscript{\cite{23}}将Mamba与视觉编码器结合, 但侧重视觉-语言理解而非动力学建模.
...
在资源受限的机器人平台上部署深度学习模型面临挑战.
知识蒸馏、模型剪枝和量化是常用压缩方法, 但通常是事后压缩, 可能导致性能损失.
从架构设计角度出发, GRU通过简化LSTM门控结构减少了参数量.
