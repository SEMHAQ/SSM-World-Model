...
消融实验表明门控机制可降低MSE约8.9\%, 序列长度分析表明输入序列长度$T{=}8$时预测精度较高.
将MIMO-WM嵌入模型预测控制框架, 交叉熵方法采样式求解器相比梯度求解器可提升控制频率1到2个数量级; MIMO-WM在Humanoid上实现了7.99Hz的控制频率, 且其3步预测均方误差为0.216, 低于各对比方法.
...
通过线性递推结构实现了$O(T)$的每步推理复杂度, 在长序列处理上具有较低的计算开销.
...
1)~~方法贡献: 提出基于维间独立状态空间与sigmoid门控的轻量级世界模型MIMO-WM, 以0.138M参数实现人形机器人状态预测, 在保持对角SSM线性计算复杂度的同时通过门控引入维度间轻量级交互.

2)~~实验贡献: 在Humanoid和HumanoidStandup两类动力学特性不同的人形机器人任务上, 系统对比了MIMO-WM与LSTM、GRU、Transformer、Mamba、TCN五种序列模型的预测性能, 验证了维度独立SSM加轻量门控在物理动力学预测中对全混合架构的优势——在Humanoid上精度优于各对比方法, 在HumanoidStandup上以Transformer的45.7\%参数量达到相当精度.

3)~~理论贡献: 证明了对角SSM递推与卷积两种计算模式的等价性, 分析了MIMO-WM较Transformer和LSTM的渐进复杂度优势, 并给出了CEM采样式MPC的收敛性保证.
...
\end{equation}
默认配置$D=96$, $N=16$, $L=2$时约0.14M, 低于各对比方法.
...
表~1展示了各模型在Humanoid数据集上的性能对比. MIMO-WM的精度优于各对比方法, MSE为0.199, 低于Mamba-WM的0.202, 低于TCN-WM的0.207, 低于Transformer-WM的0.281, 低于GRU-WM的0.366和LSTM-WM的0.399.
值得注意的是, MIMO-WM的参数量仅0.138M, 低于TCN-WM的0.189M和Mamba-WM的0.224M, 在所有方法中较为轻量.
...
综合两个数据集上的实验结果, MIMO-WM在Humanoid上的精度优于各对比方法, MSE为0.199, 比Mamba-WM的0.202低1.5\%, 比Transformer-WM的0.281低29.3\%; 在HumanoidStandup上, MIMO-WM的MSE为0.531, $R^2$为0.444, 与Transformer精度相当, 但参数量仅0.138M, 为Transformer的45.7\%, 在所有方法中较为轻量.
...
表~4表明, 两个数据集均在$T{=}8$时取得较高$R^2$, 且预测精度随$T$增大而单调下降.
